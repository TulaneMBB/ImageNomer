...
def get_correlation_plots():
    ...
    if corr_var not in sel_cohort_df.columns:
        ...
        if task == 'All':
            ...
            tasks = [task]
        state['corr_img'], state['corr_pval'], df, tot_rho, tot_pval = correlation.corr_conn_pheno(sel_cohort['name'], sel_cohort_df, corr_group, typ, tasks, corr_pheno, corr_cat)
        state['corr_stats'] = {'rho': decim(tot_rho), 'df': df, 'pval': decim(tot_pval)}
    else:
